...
def set_date_time():
    ...
    result_date = []
    while (switch):
        print "请输入计算时间的范围，格式如:2016-09-01 09:00:00; 输入#为退出本次查询".decode('utf-8')
        print "开始时间:".decode('utf-8')
        ...
        if ' ' in begin_temp:
            ...
            if begin_date_result == 0 or begin_time_result == 0:
                print "时间格式不正确，请重新输入!".decode('utf-8')
                switch = 1
            else:
                switch = 0
        else:
            print "时间格式不正确，日期与时间之间请加上空格重新输入!".decode('utf-8')
    result_date.append(begin_temp)

    switch = 1
    while (switch):
        print "结束时间:".decode('utf-8')
        end_temp = raw_input()
        if end_temp == '#':
            return 0
        if ' ' in end_temp:
